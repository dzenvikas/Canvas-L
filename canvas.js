...
function draw() {
  ...
  if (canvas.getContext) {
    ...
    //DRAWING PAC CHAR
    // reset pen and ready to draw
    ctx.beginPath();
    ctx.arc(37, 37, 13, Math.PI / 7, -Math.PI / 7, false);
    ctx.lineTo(31, 37);
    ctx.fill();

    // small rectangle dots
    for (var i = 0; i < 8; i++) {
    ...
    ctx.moveTo(103, 96);
    // LEFT EYE
    ctx.bezierCurveTo(100, 96, 99, 96, 99, 101);
...
function roundedRect(ctx, x, y, width, height, borderRadius) {
  // getting ready to draw new path
  ctx.beginPath();
  // move pen position
  ...
  ctx.arcTo(x + width, y + height, x + width, y + height - borderRadius, borderRadius);
